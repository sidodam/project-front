...
function ImageWithTitle() {
  const { data, loading, error } = useFetch(
    'https://weak-lime-sea-urchin-cap.cyclic.app/api/hotels/countByType',
  ) as {
              ...
              return (
                <div key={index}>
                  ...
                  <div className="flex justify-between mt-5">
                    <h1 className="text-[1.8rem] mb-2 capitalize">
                      {item.type}
                    </h1>
                  </div>
                  <h2 className="text-[1rem] mb-1">{`${item.count} ${
                    item.type == 'hotel' ? 'hotels' : item.type
                  }`}</h2>
                </div>
              )
            })}
