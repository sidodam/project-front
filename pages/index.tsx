...
const Home: NextPage = () => {
  ...
  const { data: datos } = useFetch(
    'https://weak-lime-sea-urchin-cap.cyclic.app/api/contactus',
  )

  const { data: nextDest } = useFetch(
    'https://weak-lime-sea-urchin-cap.cyclic.app/api/hotels',
  )
  ...
  const handleClick = () => {
    router.push(`/discover?destination=${nextDest[x]?.city}`)
  }
